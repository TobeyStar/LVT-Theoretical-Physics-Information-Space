...
\subsection{3.1 The Informational Cost ($E_{info}$)}
Once the payload's complexity ($C_{\mathrm{eff}}$) is successfully isolated below the local rendering threshold ($T_{\mathrm{local}}$) via the SRC cavity, the system's external node-pointer ($LV_{\mathrm{external}}$) enters a floating, undefined state.

To execute the Topological Translocation, the navigation computer (NavCom) must inject a new destination address ($LV_B$) into the local matrix buffer. According to the Landauer-Hawking framework, overwriting this spatial data is a physically irreversible process that demands a strict thermodynamic exhaust. 

The informational work required to rewrite the macroscopic pointer is directly proportional to the bits erased from the Host's cosmic horizon. Utilizing the established empirical quantum of spatial erasure ($E_{\mathrm{Landauer}}$), the rewriting cost is:
\begin{equation}
E_{\mathrm{info}} = N_{\mathrm{bits}} \cdot E_{\mathrm{Landauer}} = N_{\mathrm{bits}} \cdot \left( 1.74 \times 10^{-30} \text{ J} \right)
\end{equation}

Unlike kinetic propulsion, where energy scales exponentially with physical distance ($S$), $E_{\mathrm{info}}$ scales strictly with the logical data-size of the target coordinates. The translation cost is an administrative matrix fee, completely decoupled from the spatial metric.
